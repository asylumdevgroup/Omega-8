...
IF NOT EXIST "%cd%\serverstarter-2.4.0.jar" (
	...
	%SYSTEMROOT%\SYSTEM32itsadmin.exe /rawreturn /nowrap /transfer starter /dynamic /download /priority foreground cdn.naylahanegan.com/serverstarter-2.4.0.jar "%cd%\serverstarter-2.4.0.jar"
   GOTO MAIN
) ELSE (
   GOTO MAIN
)
